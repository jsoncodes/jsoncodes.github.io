...
function CompactArticleSummary({ title, subject, date, coverImage, link }: Post) {
  return (
...
function FeaturedArticleSummary({ title, subject, excerpt, date, coverImage, link }: Post) {
  return (
    <ArticleLink href={link}>
      <FeaturedRoot>
...
export function ArticleSummary({ variant, ...props }: ArticleProps) {
  if (variant === 'compact') {
